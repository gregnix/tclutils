...
-word 0|1               match only whole words (like grep -w)
-wholeline 0|1          match only when the whole line matches (like grep -x)
-max N                  stop after N matching lines, 0 = unlimited (like grep -m)
